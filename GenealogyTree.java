...
public class GenealogyTree{
	...
	public void buildFromFile(String filename) throws IOException{
		...
		// Create a queue, add each new node to the queue
		Queue loadQ = new Queue();
			// create a Scanner connect to the file
			Scanner in = null;

			// If the file that is loaded doesn't match any found in the
			// directory, throw an IOException
			try {
			...
			while(in.hasNextLine()){
				...
				// else Construct other TreeNode
						...
						TreeNode tnode = new TreeNode(parts[1]);
					
					// while queue is not empty
						while(!loadQ.isEmpty()){

						// get next node from queue without removing it from queue
						TreeNode temp = (TreeNode) loadQ.element();
						// if "front" node matches the parent
							if(temp.getData().equals(parts[0])){
								temp.addChild(tnode);
