...
public class LinkedList<E> implements ListADT<E> {
	...
	public boolean contains(E item) {
		...
		LinkedListIterator<E> itr = (LinkedListIterator<E>) iterator();
		...
		if (item instanceof String)
		{
			while (itr.hasNext())
			{
				...
				if (( checker).getData().equals(item))
					return true;
				}
		}
		else
		{
			Listnode<E> newItem =  (Listnode<E>) item;

		while (itr.hasNext())
		{
			Listnode<E> checker = (Listnode<E>) itr.next();

			if (checker.getData().equals(newItem.getData()))
				return true;
			}
		}
